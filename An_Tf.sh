#!/bin/bash
# Global vars
dir="$HOME/YOUR_LOCAL_REPO"
date=$(date +%H-%M-%S)
filename=$(basename -- "$0" | cut -d '.' -f 1)
iac_dir="$HOME/$filename"
prod_live_id="i-XXXXXXX_YOUR_ID_NOT_EXPOSED_XXXXXXX"
ec2_info() {
...
# Waiting for a minute and a half
init() {
sleep 90 &
p=$!
f="/ | \\ -"
...
data "aws_vpc" "YOUR_NAME_vpc" {
  tags = {
    Name = "YOUR_VPC_NAME"
  }
}
resource "aws_subnet" "subnet-jenkins_docker" {
  vpc_id = data.aws_vpc.YOUR_NAME_vpc.id
  availability_zone = data.aws_availability_zones.avail.names[0]
  cidr_block = "172.31.96.0/24" # Your CIDR block
  tags = {
...
resource "aws_security_group" "sg-jenkins_docker" {
  ...
  ingress {
    ...
    to_port          = 22
    protocol         = "tcp"
    cidr_blocks      = ["xxx.xxx.xxx.xxx/32"] # Your IP
  }
  egress {
    from_port        = 0
    to_port          = 0
  ...
  tasks:
    ...
    - name: Create Aws Creds
      ansible.builtin.copy:
        ...
        content: |
          [default]
          AWS_ACCESS_KEY_ID=XXXXXXX_YOUR_CREDS_NOT_EXPOSED_XXXXXXX
          AWS_SECRET_ACCESS_KEY=XXXXXXX_YOUR_CREDS_NOT_EXPOSED_XXXXXXX
        mode: '0600'
        owner: ec2-user
        group: ec2-user
    ...
    - name: Create GH Token file
      ansible.builtin.copy:
        dest: "{{ dest_token }}"
        content: 'ghp_XXXXXXX_YOUR_TOKEN_NOT_EXPOSED_XXXXXXX'
        owner: ec2-user
        group: ec2-user
        mode: '600'
    - name: GH CLI Auth
    ...
    - name: Set GitHub Webhook
      ...
      async: 45
      poll: 0
    - name: Test Git push at localhost
      become_user: a
      ansible.builtin.shell: |
        cd $dir
        touch Ansible-$date
        git remote remove origin
        git remote add origin https://ghp_TOKEN@github.com/YOUR_USER/YOUR_REPO.git
        git add . && git commit -m "Pushed by Ansible" && git push --all origin
      register: git
...
ansible_ssh_private_key_file: ~/.ssh/PATH_TO_YOUR_KEY.pem
EOF
...
#####################################################
# Some Shell script stuff goes here                 #
# like doing backups etc.                           #
# See more at: https://github.com/HarrierPanels/sql #
#####################################################
# CI/CD
cicd_toolchain